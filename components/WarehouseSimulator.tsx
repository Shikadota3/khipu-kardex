...
function getTabla10Code(type: string) {
  ...
  // Si ya es un código de dos dígitos, lo devolvemos
...
export default function WarehouseSimulator() {
  ...
  const resumenMensual = React.useMemo(() => {
    ...
    // Ejecutar motor para el activo seleccionado
    ...
    kardexCompleto.forEach(line => {
      ...
      // El último registro de cada mes contiene el saldo final de ese mes
      meses[key] = line;
    });
  ...
  const [nuevoActivo, setNuevoActivo] = useState<any>({
    ...
    unidadMedidaCodigo: '07', // UNIDADES
    tipoExistencia: '01',     // MERCADERÍA
    stockMinimo: 10, 
    stockMaximo: 100, 
    observaciones: '', 
    initialStock: 0, 
  ...
  // --- ESTADOS LOCALES PARA ENTRADA BINARIA DE CANTIDAD, COSTO Y CÁLCULOS DE IGV ---
  ...
  const [localGrabado, setLocalGrabado] = useState<boolean>(true); // true = Grabado (18% IGV), false = No Grabado (Exonerado 0% IGV)
  ...
  // --- CONTROLADORAS DE CÁLCULO TRIPLE CON IGV (AFECTACIÓN DINÁMICA 18% / 0%) ---
  const recalculateFromSin = (sinStr: string, qtyStr: string, isGrabado: boolean, source?: 'costoSin' | 'costoCon' | 'totalCon' | 'cantidad' | 'grabado') => {
    ...
    if (parsedQty > 0) {
      ...
      if (source !== 'costoCon') {
        setLocalCostoConIGV(sinStr === '' ? '' : conIGV.toFixed(4));
      }
      if (source !== 'costoSin') {
        setLocalCostoSinIGV(sinStr === '' ? '' : sinIGV.toFixed(4));
      }
      if (source !== 'totalCon') {
        setLocalTotalConIGV(sinStr === '' || qtyStr === '' ? '' : roundedTotal.toFixed(2));
      }
      
      setNuevaOperacion(prev => ({ 
        ...prev, 
        cantidad: parsedQty, 
        costoUnitario: sinIGV 
      }));
    } else {
      ...
      if (source !== 'costoSin') {
        setLocalCostoSinIGV(sinStr === '' ? '' : parsedSin.toFixed(4));
      }
      if (source !== 'totalCon') {
        setLocalTotalConIGV('');
      }
  ...
  const recalculateFromCon = (conStr: string, qtyStr: string, isGrabado: boolean, source?: 'costoSin' | 'costoCon' | 'totalCon' | 'cantidad' | 'grabado') => {
    ...
    if (parsedQty > 0) {
      ...
      if (source !== 'costoCon') {
        setLocalCostoConIGV(conStr === '' ? '' : conIGV.toFixed(4));
      }
      if (source !== 'costoSin') {
        setLocalCostoSinIGV(conStr === '' ? '' : sinIGV.toFixed(4));
      }
      if (source !== 'totalCon') {
        setLocalTotalConIGV(conStr === '' || qtyStr === '' ? '' : roundedTotal.toFixed(2));
      }
      ...
      if (source !== 'costoCon') {
        setLocalCostoConIGV(conStr === '' ? '' : parsedCon.toFixed(4));
      }
      if (source !== 'totalCon') {
        setLocalTotalConIGV('');
      }
  ...
  const recalculateFromTotal = (totalStr: string, qtyStr: string, isGrabado: boolean, source?: 'costoSin' | 'costoCon' | 'totalCon' | 'cantidad' | 'grabado') => {
    ...
    if (parsedQty > 0) {
      ...
      if (source !== 'costoCon') {
        setLocalCostoConIGV(totalStr === '' ? '' : conIGV.toFixed(4));
      }
      if (source !== 'costoSin') {
        setLocalCostoSinIGV(totalStr === '' ? '' : sinIGV.toFixed(4));
      }
      if (source !== 'totalCon') {
        setLocalTotalConIGV(totalStr === '' ? '' : parsedTotal.toFixed(2));
      }
  ...
  // --- PERSISTENCIA DE DATOS (KHIPU STORAGE) ---
  useEffect(() => {
    if (nuevaOperacion.tipo === 'ENTRADA') {
      ...
      setNuevaOperacion(prev => ({ ...prev, tipoOperacion: '01' })); // VENTA
    }
  }, [nuevaOperacion.tipo]);

  // Forzar sin IGV para salidas que no sean Factura ni Boleta
  useEffect(() => {
  ...
  useEffect(() => {
    const usernameKey = currentUser?.username || 'invitado';
    const savedProducts = localStorage.getItem(`kardex_products_${usernameKey}`);
    const savedMovements = localStorage.getItem(`kardex_movements_${usernameKey}`);
    const savedMethod = localStorage.getItem(`kardex_valuation_method_${usernameKey}`);
    const savedEmpresa = localStorage.getItem(`kardex_empresa_${usernameKey}`);
    
    const p = savedProducts ? JSON.parse(savedProducts) : [];
    const m = savedMovements ? JSON.parse(savedMovements) : [];
    const method = savedMethod ? savedMethod as ProtocoloValuacion : 'PROMEDIO';
    const empresa = savedEmpresa ? JSON.parse(savedEmpresa) : null;
    
    if (empresa) setEmpresaConfig(empresa);
    
    setMaestroActivos(p);
    setBitacoraOperaciones(m);
    setProtocoloActivo(method);
    setDataLoaded(true);
    setMounted(true);
  }, [currentUser]);
  ...
  const guardarEmpresa = (data: typeof empresaConfig) => {
    ...
    const usernameKey = currentUser?.username || 'invitado';
    localStorage.setItem(`kardex_empresa_${usernameKey}`, JSON.stringify(config));
  };

  useEffect(() => {
    if (mounted && dataLoaded) {
      const usernameKey = currentUser?.username || 'invitado';
      localStorage.setItem(`kardex_products_${usernameKey}`, JSON.stringify(maestroActivos));
      localStorage.setItem(`kardex_movements_${usernameKey}`, JSON.stringify(bitacoraOperaciones));
      localStorage.setItem(`kardex_valuation_method_${usernameKey}`, protocoloActivo);
    }
  }, [maestroActivos, bitacoraOperaciones, protocoloActivo, mounted, dataLoaded, currentUser]);
  ...
  const registrarNuevoActivo = () => {
    ...
    // Validación de Unicidad de Código (Regla SUNAT/KHIPU)
    ...
    // Validación de Inmutabilidad del Protocolo (Regla Crítica KHIPU)
    if (!esKardexVacio) {
      const usernameKey = currentUser?.username || 'invitado';
      const savedMethod = localStorage.getItem(`kardex_valuation_method_${usernameKey}`);
    ...
    if (stockInicialVal > 0) {
      const opInicial: OperacionKardex = {
        ...
        tipoDocumento: '00', // OTROS
        tipoOperacion: '16', // SALDO INICIAL
        serie: 'INI',
        numero: '001',
  ...
  const agregarPartidaAStaging = () => {
    if (!activoSeleccionadoId) {
      alert("Seleccione un activo.");
      return;
    }
    if ((nuevaOperacion.cantidad || 0) <= 0) {
      alert("Ingrese una cantidad válida.");
      return;
    }
    if (nuevaOperacion.tipo === 'ENTRADA' && (nuevaOperacion.costoUnitario || 0) <= 0) {
      alert("Ingrese un costo unitario válido.");
      return;
    }

    // Validación de stock para salidas en staging
    if (nuevaOperacion.tipo === 'SALIDA') {
      const saldoActual = obtenerSaldoFisico(activoSeleccionadoId);
      // Considerar otros items del mismo activo ya en staging
    ...
    const nuevaPartida = {
      ...
      igvGrabado: localGrabado, // Guardar indicador de gravado/no gravado
    };

    setStagedItems([...stagedItems, nuevaPartida]);
    // Limpiar campos específicos del activo y del estado binario para el siguiente item
    setNuevaOperacion(prev => ({
      ...prev,
      cantidad: 0,
      costoUnitario: 0,
  ...
  const descargarPDFComprobante = () => {
    ...
    // Configuración de Colores y Estilos (Estética Khipu)
    const primaryColor = [43, 87, 154] as [number, number, number]; // #2B579A
    const accentColor = [249, 115, 22] as [number, number, number];  // #F97316
    ...
    // Cuadro de Información (Derecha) - Movido abajo o mejor espaciado
    ...
    // Datos Adicionales con mejor espaciado vertical
    ...
    doc.setFont('helvetica', 'normal');
    const tipoOpStr = nuevaOperacion.tipo === 'ENTRADA' ? 'COMPRA / ENTRADA' : 'VENTA / SALIDA';
    doc.text(tipoOpStr, 60, 68);
    ...
    // Tabla de Items (Iniciamos en 82 para dar excelente aire y evitar empastes)
    ...
    const tableData = stagedItems.map(item => {
      ...
      const subtotalItem = item.costoTotal; // cantidad * costoUnitario (base imponible)
    ...
    // Cálculos de IGV desagregados (Gravada, No Gravada, IGV) por cada ítem
    ...
    // Resumen de Totales con mayor claridad vertical y sangría
    doc.setFontSize(9.5);
  ...
  const ejecutarAjusteAutomatico = () => {
    ...
    maestroActivos.forEach(p => {
      ...
      if (diferencia !== 0) {
        ...
        // Base Unit Cost
        let costoUnitario = lastLine ? lastLine.costoUnitarioSaldo : p.costoInicial || 0;
    ...
    if (countAdj === 0) {
      alert("No hay diferencias reportadas en los activos listados.");
      return;
    }
  ...
  const agregarAjusteAManualStaging = (prodId: string, diferencia: number) => {
    ...
    let costoUnitario = lastLine ? lastLine.costoUnitarioSaldo : p.costoInicial || 10.0;
    if (costoUnitario <= 0) costoUnitario = 10.0;
  ...
  /**
   * MANEJADOR_DE_EJECUCIÓN (handleExecute): Procesa el asentamiento contable.
   */
  const asentarOperacionKardex = () => {
    console.log(">>> [KHIPU_SYSTEM]: Iniciando proceso de asentamiento...");
    
    // Si hay items en staging, los procesamos todos
    ...
    // Si no hay items en staging, intentamos procesar el item actual directamente
    // (Mantenemos esta lógica para compatibilidad o entradas rápidas de un solo item)
    const cantidadRaw = nuevaOperacion.cantidad;
    
    const cantidad = parseFloat(String(cantidadRaw)) || 0;
    ...
    // ... (rest of validations duplicate logic but since we want to encourage staging, 
    // maybe we just tell them to add it to the list first?)
    // Actually, let's just make it simple: 
    // if stagedItems is empty, we "add" it and then commit.
  ...
  const confirmarEliminacionKhipu = () => {
    ...
    if (deleteConfirm.type === 'product' && deleteConfirm.id) {
      ...
      if (activoSeleccionadoId === deleteConfirm.id) {
        setActivoSeleccionadoId('');
      }
      ...
      setActivoSeleccionadoId('');
      const usernameKey = currentUser?.username || 'invitado';
      localStorage.removeItem(`kardex_products_${usernameKey}`);
      localStorage.removeItem(`kardex_movements_${usernameKey}`);
    }
  ...
  const exportToExcel = () => {
    ...
    const data = kardex.map(line => ({
      Fecha: line.fecha,
      Tipo: line.tipoDocumento,
      Serie: line.serie,
      Número: line.numero,
      Operación: line.tipo,
      'Entrada Cant': line.cantidadEntrada || '',
      'Entrada Costo U': line.costoUnitarioEntrada || '',
      'Entrada Total': line.costoTotalEntrada || '',
      'Salida Cant': line.cantidadSalida || '',
      'Salida Costo U': line.costoUnitarioSalida || '',
      'Salida Total': line.costoTotalSalida || '',
      'Saldo Cant': line.cantidadSaldo,
      'Saldo Costo U': line.costoUnitarioSaldo,
      'Saldo Total': line.costoTotalSaldo,
    }));
  ...
  const exportToPDF = () => {
    ...
    // Header SUNAT Format 12.1 - KHIPU Standard
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    ...
    autoTable(doc, {
      ...
      body: kardex.map(l => [
        l.fecha, 
        getTabla10(l.tipoDocumento), 
        l.serie, 
        l.numero, 
        l.tipoOperacion || (l.tipo === 'ENTRADA' ? '02' : '01'),
        l.cantidadEntrada || '-', 
        l.cantidadSalida || '-', 
        l.cantidadSaldo
      ]),
      ...
      columnStyles: {
        ...
        1: { cellWidth: 25, halign: 'center' }, // Tipo (Tabla 10)
        2: { cellWidth: 20, halign: 'center' }, // Serie
        3: { cellWidth: 35, halign: 'center' }, // Número
        4: { cellWidth: 35, halign: 'center' }, // Operación (Tabla 12)
        5: { cellWidth: 35, halign: 'right' }, // Entradas
        6: { cellWidth: 35, halign: 'right' }, // Salidas
        7: { cellWidth: 35, halign: 'right' }  // Saldo Final
      }
    });
  ...
  const exportToPDFValorizado = () => {
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        1: { cellWidth: 15, halign: 'center' }, // Tipo
        2: { cellWidth: 15, halign: 'center' }, // Serie
        3: { cellWidth: 22, halign: 'center' }, // Número
        4: { cellWidth: 18, halign: 'center' }, // Operación
        5: { cellWidth: 19, halign: 'right' }, // Ent. Cant
        6: { cellWidth: 19, halign: 'right' }, // Ent. Costo U
        7: { cellWidth: 19, halign: 'right' }, // Ent. Costo T
        8: { cellWidth: 19, halign: 'right' }, // Sal. Cant
        9: { cellWidth: 19, halign: 'right' }, // Sal. Costo U
        10: { cellWidth: 19, halign: 'right' }, // Sal. Costo T
        11: { cellWidth: 19, halign: 'right' }, // Saldo Cant
        12: { cellWidth: 19, halign: 'right' }, // Saldo Costo U
        13: { cellWidth: 19, halign: 'right' }  // Saldo Costo T
      }
    });
  ...
  const exportToExcelSUNAT121 = () => {
    ...
    const header = [
      ...
      [`RUC: ${empresaConfig.ruc || '20601234567'}`],
      [`APELLIDOS Y NOMBRES, DENOMINACIÓN O RAZÓN SOCIAL: ${(empresaConfig.nombre || 'KHIPU').toUpperCase()}`],
      ['ESTABLECIMIENTO: ALMACÉN CENTRAL'],
      [`CÓDIGO DE LA EXISTENCIA: ${activoSeleccionado.codigo}`],
      [`TIPO (TABLA 5): ${activoSeleccionado.tipoExistencia || '01'}`],
      [`DESCRIPCIÓN: ${activoSeleccionado.nombre.toUpperCase()}`],
  ...
  const exportToExcelSUNAT131 = () => {
    ...
    const rows = kardex.map(l => [
      ...
      l.tipoOperacion || (l.tipo === 'ENTRADA' ? '02' : '01'),
      l.cantidadEntrada || 0,
      l.costoUnitarioEntrada || 0,
      l.costoTotalEntrada || 0,
      l.cantidadSalida || 0,
      l.costoUnitarioSalida || 0,
      l.costoTotalSalida || 0,
      l.cantidadSaldo,
      l.costoUnitarioSaldo,
      l.costoTotalSaldo
    ]);
  ...
  const exportStockAlertsReport = () => {
    const alerts = maestroActivos.filter(p => {
      ...
      return {
        ...
        'Estado': stock <= p.stockMinimo ? 'STOCK BAJO' : 'SOBRESTOCK'
      };
    });

    if (alerts.length === 0) {
      alert("No hay alertas de stock para exportar.");
      return;
    }
  ...
  const exportLowMovementReport = () => {
    ...
    const lowMovementProducts = maestroActivos.filter(p => {
      ...
      'Código': p.codigo,
      'Producto': p.nombre,
      'Stock Actual': obtenerSaldoFisico(p.id),
      'Último Movimiento': 'Sin movimientos este mes'
    }));

    if (lowMovementProducts.length === 0) {
      alert("No hay productos con poco movimiento para exportar.");
      return;
    }
    ...
    autoTable(doc, {
      ...
      body: maestroActivos.map(p => {
        ...
        return [
          ...
          getTabla5Label(p.tipoExistencia),
          p.unidadMedida,
          totalEntradas,
          totalSalidas,
          stockActual,
  ...
  const exportMaestroArticulosExcel = () => {
    const data = maestroActivos.map(p => {
      ...
      return {
        'Código': p.codigo,
        'Descripción': p.nombre.toUpperCase(),
        'Tipo (Tabla 5)': getTabla5Label(p.tipoExistencia),
        'Unidad de Medida': p.unidadMedida,
        'Total Entradas': totalEntradas,
        'Total Salidas': totalSalidas,
        'Stock Actual': lastLine ? lastLine.cantidadSaldo : 0,
        'Costo Unitario': lastLine ? lastLine.costoUnitarioSaldo : 0,
        'Valorización Total': lastLine ? lastLine.costoTotalSaldo : 0,
        'Stock Mínimo': p.stockMinimo,
        'Stock Máximo': p.stockMaximo,
  ...
  const descargarPlantillaMasiva = () => {
    const template = [
      {
        'CÓDIGO': 'P-001',
        'DESCRIPCIÓN': 'ARTÍCULO DE EJEMPLO',
        'TIPO_SUNAT': '01',
        'UNIDAD_SUNAT': '07',
        'STOCK_MIN': 10,
        'STOCK_MAX': 100,
        'STOCK_INICIAL': 0,
        'COSTO_UNIT_INICIAL': 0
      }
    ];
    ...
    reader.onload = (event) => {
      try {
        ...
        jsonData.forEach((row) => {
          ...
          if (maestroActivos.some(a => a.codigo.toUpperCase() === codigo.toUpperCase()) || nuevosActivos.some(a => a.codigo.toUpperCase() === codigo.toUpperCase())) {
            return;
          }

          const productId = crypto.randomUUID();
          
          // Obtener descripción de unidad desde el código
          ...
          const activo: ActivoKhipu = {
            ...
            codigo: codigo,
            nombre: nombre,
            unidadMedida: tabla6Map[unidadCodigo] || 'UNIDADES',
            unidadMedidaCodigo: unidadCodigo,
            tipoExistencia: String(row['TIPO_SUNAT'] || '01').padStart(2, '0'),
            stockMinimo: Number(row['STOCK_MIN']) || 0,
            stockMaximo: Number(row['STOCK_MAX']) || 0,
  ...
  return (
    ...
    <Image 
        ...
        <div className="mt-auto p-6 border-t border-[#DEE2E6] space-y-3">
          <button 
            onClick={inyectarDatosDemo}
            className="w-full py-2 px-3 border border-[#DEE2E6] rounded-none text-[9px] font-sans font-bold text-[#666] hover:border-[#2B579A] hover:text-[#2B579A] transition-all uppercase tracking-tighter"
          >
        ...
        <div className="h-16 border-b border-[#DEE2E6] flex items-center justify-between px-12 bg-white sticky top-0 z-20">
          <div className="flex items-center gap-6">
            ...
            <button 
              ...
              className="flex items-center gap-2 p-2 hover:bg-red-50 text-[#999] hover:text-red-500 transition-all border border-transparent hover:border-red-100"
              title="Cerrar Sesión"
            >
              <LogOut size={16} />
              ...
              <>
                <div className="relative">
                  ...
                  <input 
                    ...
                    value={terminoBusquedaActivo}
                    onChange={(e) => setTerminoBusquedaActivo(e.target.value)}
                    className="pl-11 pr-4 py-3 bg-white border border-[#DEE2E6] rounded-none text-xs font-sans text-[#333] focus:outline-none focus:border-[#2B579A] transition-all w-64"
                  />
                </div>
                {(currentUser?.role === 'ADMIN' || currentUser?.role === 'STUDENT') && (
                  <button 
                    onClick={() => {
                      setNuevoActivo({ ...nuevoActivo, codigo: recomendarCodigo() });
                      setShowProductModal(true);
                    }}
                    className="bg-[#2B579A] text-white px-6 py-3 rounded-none flex items-center gap-2 hover:bg-[#1A3A6A] transition-all text-[11px] font-sans font-bold uppercase tracking-wider shadow-sm"
                  >
                    <Plus size={16} />
                    Añadir Activo
                  </button>
            ...
            {activeTab === 'kardex' && (
              <div className="flex gap-4">
                {(currentUser?.role === 'ADMIN' || currentUser?.role === 'STUDENT') && (
                  <button 
                    onClick={() => {
                      setNuevaOperacion({...nuevaOperacion, tipoDocumento: 'AJUSTE', serie: 'ADJ', numero: format(new Date(), 'yyyyMMdd')});
                      setShowMovementModal(true);
                    }}
                    className="bg-orange-50 text-[#F97316] border border-orange-200 px-6 py-3 rounded-none flex items-center gap-2 hover:bg-[#F97316] hover:text-white transition-all text-[11px] font-sans font-bold uppercase tracking-wider shadow-sm"
                  >
                    <Settings size={16} />
          ...
          {activeTab === 'dashboard' && (
            ...
            >
              <StatCard 
                ...
                value={maestroActivos.length} 
                subtext="Ítems registrados" 
                colorClass="bg-[#E7EEF8] text-[#2B579A]"
              />
              <StatCard 
                icon={ClipboardList} 
                label="Ops Mensuales" 
                value={bitacoraOperaciones.filter(m => {
                  const d = safeDate(m.fecha);
              ...
              <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-8">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      <thead className="text-[9px] uppercase tracking-widest text-[#999] font-sans font-bold border-b border-[#DEE2E6]">
                        <tr>
                          <th className="pb-4">Nombre_Activo</th>
                          <th className="pb-4">Cant</th>
                          <th className="pb-4">Valorización_S/</th>
                        </tr>
                      </thead>
                        ...
                        {maestroActivos.map(p => {
                          ...
                          return (
                            <tr key={p.id} className="group hover:bg-gray-50 transition-colors">
                              <td className="py-4">
                                <div className="font-bold text-[#333]">{p.nombre}</div>
                                <div className="text-[9px] text-[#999]">{p.codigo}</div>
                              </td>
                              <td className="py-4">
                                <div className="text-[#333]">{stock} {p.unidadMedida}</div>
                                {stock <= p.stockMinimo && <span className="text-[8px] font-bold text-[#F97316] uppercase tracking-tighter">Stock_Bajo</span>}
                              </td>
                              <td className="py-4 text-[#2B579A] font-bold">
                                {totalValue.toLocaleString('es-PE', { minimumFractionDigits: 2 })}
                              </td>
                            </tr>
                          );
                        })}
                        {maestroActivos.length === 0 && (
                          <tr>
                            <td colSpan={3} className="py-10 text-center text-[#999] italic">No hay activos registrados.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
                ...
                <Card className="p-8">
                  ...
                  <div className="space-y-4">
                    {bitacoraOperaciones.slice(-5).reverse().map((m, idx) => {
                      ...
                      return (
                        <div key={m.id || idx} className="flex items-center justify-between p-4 border border-[#DEE2E6] bg-[#F8F9FA] group hover:border-[#2B579A]/30 transition-all">
                          ...
                          <div className="text-right">
                            <p className={`text-[11px] font-bold ${m.tipo === 'ENTRADA' ? 'text-[#2B579A]' : 'text-[#F97316]'}`}>
                    ...
                    {bitacoraOperaciones.length === 0 && (
                      <div className="py-12 text-center border border-dashed border-[#DEE2E6]">
                        <p className="text-[10px] text-[#999] uppercase tracking-widest">Sin_Movimientos_Recientes</p>
                      </div>
                    )}
                  </div>
                </Card>
                  ...
                  <div className="space-y-4">
                    {maestroActivos.filter(p => obtenerSaldoFisico(p.id) <= p.stockMinimo).slice(0, 5).map(p => (
                      <div key={p.id} className="flex items-center justify-between p-4 border border-orange-100 bg-orange-50/50 group hover:bg-orange-50 transition-all">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 flex items-center justify-center bg-orange-100 text-[#F97316]">
                        ...
                        <div className="text-right">
                          <p className="text-[11px] font-bold text-[#F97316]">
                            {obtenerSaldoFisico(p.id)} {p.unidadMedida}
                          </p>
                          <p className="text-[9px] text-[#999] uppercase tracking-widest">Crítico</p>
                        </div>
                      </div>
                    ))}
                    {maestroActivos.filter(p => obtenerSaldoFisico(p.id) <= p.stockMinimo).length === 0 && (
                      <div className="py-12 text-center border border-dashed border-[#DEE2E6]">
                        <p className="text-[10px] text-[#999] uppercase tracking-widest">Sin_Alertas_Criticas</p>
                      </div>
                    )}
                  </div>
                </Card>
            ...
            >
              ...
              <div className="bg-white p-8 border border-[#DEE2E6] shadow-sm flex justify-between items-center">
                <div className="relative max-w-md w-full">
                  ...
                  <input 
                    type="text"
                    placeholder="Filtrar por nombre o código en catálogo..."
                    value={terminoBusquedaActivo}
                    onChange={(e) => setTerminoBusquedaActivo(e.target.value)}
                ...
                <div className="flex gap-3">
                  {(currentUser?.role === 'ADMIN' || currentUser?.role === 'STUDENT') && (
                    <>
                      ...
                      <button 
                        ...
                        className="flex items-center gap-2 px-4 py-3 border border-[#DEE2E6] bg-white text-[#999] text-[9px] font-sans font-bold hover:bg-gray-50 transition-all uppercase tracking-widest"
                      ...
                      <label 
                        ...
                        className="flex items-center gap-2 px-6 py-3 border border-[#2B579A] bg-[#E7EEF8] text-[#2B579A] text-[9px] font-sans font-bold hover:bg-[#2B579A] hover:text-white transition-all uppercase tracking-widest cursor-pointer"
                      >
                        <Upload size={14} />
                        Carga_Masiva
                      </label>
                  ...
                  <button 
                    onClick={exportMaestroArticulosPDF}
                  ...
                  <button 
                    onClick={exportMaestroArticulosExcel}
                    className="flex items-center gap-2 px-6 py-3 border border-[#DEE2E6] bg-white text-[#333] text-[9px] font-sans font-bold hover:bg-[#2B579A] hover:text-white transition-all uppercase tracking-widest"
                  >
                    <Download size={14} />
                    Reporte_Maestro_XLS
                  ...
                  <button 
                    onClick={() => {
                      ...
                      maestroActivos.forEach(p => {
                        initialCounts[p.id] = obtenerSaldoFisico(p.id);
                      });
                      setPhysicalCounts(initialCounts);
                      setAdjustmentNumero(Math.floor(100000 + Math.random() * 900000).toString());
                      setShowAdjustmentModal(true);
                    }}
                    className="flex items-center gap-2 px-6 py-3 border border-orange-500 bg-orange-50 text-orange-600 text-[9px] font-sans font-bold hover:bg-orange-600 hover:text-white transition-all uppercase tracking-widest shadow-sm"
                  >
                    <Settings size={14} />
                    Toma_Física_Ajuste
                  </button>
              ...
              <div className="bg-white border border-[#DEE2E6] shadow-sm overflow-hidden">
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="bg-[#F8F9FA] text-[9px] uppercase tracking-[0.2em] text-[#999] font-sans font-bold border-b border-[#DEE2E6]">
                    <tr>
                      <th className="px-8 py-5">ID_Activo</th>
                      <th className="px-8 py-5">Descripción</th>
                      <th className="px-8 py-5">Tipo</th>
                      <th className="px-8 py-5">Unidad</th>
                      <th className="px-8 py-5">Límites</th>
                  ...
                  <tbody className="divide-y divide-[#DEE2E6] text-[11px] font-sans">
                    {activosFiltrados.map(p => {
                      ...
                      return (
                        <tr key={p.id} className="hover:bg-gray-50 transition-colors group">
                          ...
                          <td className="px-8 py-5 text-[#999] text-[9px] font-bold uppercase tracking-tighter">
                            {getTabla5Label(p.tipoExistencia)}
                          </td>
                          ...
                          <td className="px-8 py-5">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="h-1 w-16 bg-[#DEE2E6] rounded-none overflow-hidden">
                                <div 
                                  className="h-full bg-[#2B579A]" 
                                  style={{ width: `${Math.min(100, (stock / p.stockMaximo) * 100)}%` }}
                                ></div>
                              </div>
                              <span className="text-[9px] font-bold text-[#999]">{p.stockMaximo}</span>
                            </div>
                          </td>
                          <td className="px-8 py-5 text-[#333] font-mono">
                            S/ {unitCost.toLocaleString('es-PE', { minimumFractionDigits: 2, maximumFractionDigits: 4 })}
                          </td>
                          <td className="px-8 py-5">
                            <span className={`text-xl font-bold ${stock <= p.stockMinimo ? 'text-[#F97316]' : 'text-[#2B579A]'}`}>
                              {stock}
                            </span>
                          </td>
                    ...
                    {maestroActivos.length > 0 && activosFiltrados.length === 0 && (
                      <tr>
                        <td colSpan={6} className="px-8 py-32 text-center text-[#999]">
                          <div className="max-w-xs mx-auto">
                            <div className="w-16 h-16 border border-[#DEE2E6] bg-[#F8F9FA] flex items-center justify-center mx-auto mb-6">
                              <Search size={32} className="text-[#DEE2E6]" />
                            </div>
                            <h5 className="font-sans font-bold text-xl text-[#2B579A] mb-2 uppercase tracking-tighter">Sin_Coincidencias</h5>
                            <p className="text-[10px] leading-relaxed uppercase tracking-tight">La búsqueda &quot;{terminoBusquedaActivo}&quot; retornó 0 resultados.</p>
                          </div>
                        </td>
                      </tr>
                    )}
                      ...
                      <tr>
                        <td colSpan={6} className="px-8 py-32 text-center text-[#999]">
                          <div className="max-w-xs mx-auto">
                            ...
                            <h5 className="font-sans font-bold text-xl text-[#2B579A] mb-2 uppercase tracking-tighter">Registro_Vacío</h5>
                            <p className="text-[10px] leading-relaxed uppercase tracking-tight">Inicie el sistema añadiendo activos a la base de datos primaria.</p>
                          </div>
                        </td>
                      </tr>
                    )}
            ...
            >
                  ...
                  <div className="flex gap-3">
                    <div className="relative flex-1">
                      ...
                      <input 
                        ...
                        value={terminoBusquedaKardex}
                        onChange={(e) => setTerminoBusquedaKardex(e.target.value)}
                        className="w-full pl-11 pr-4 py-3 bg-white border border-[#DEE2E6] rounded-none text-[11px] font-sans text-[#333] focus:outline-none focus:border-[#2B579A] transition-all placeholder:text-[#DEE2E6]"
                      />
                    </div>
                    <select 
                      value={activoSeleccionadoId}
                      onChange={(e) => setActivoSeleccionadoId(e.target.value)}
                      className="flex-1 bg-white border border-[#DEE2E6] rounded-none px-4 py-3 text-[11px] font-sans text-[#333] focus:outline-none focus:border-[#2B579A] transition-all appearance-none cursor-pointer"
                    >
                      <option value="">-- SELECCIONAR ACTIVO --</option>
                      {maestroActivos
                        .filter(p => 
                          (p.nombre || '').toLowerCase().includes((terminoBusquedaKardex || '').toLowerCase()) || 
                          (p.codigo || '').toLowerCase().includes((terminoBusquedaKardex || '').toLowerCase())
                        )
                        .map(p => (
                ...
                <div>
                  ...
                  <div className={`flex bg-[#F8F9FA] p-1 border border-[#DEE2E6] ${!esKardexVacio ? 'opacity-70 grayscale-[0.5]' : ''}`}>
                    {(['PROMEDIO', 'PEPS'] as ProtocoloValuacion[]).map((m) => (
                      <button
                        key={m}
                        disabled={!esKardexVacio}
                        onClick={() => setProtocoloActivo(m)}
                        className={`px-4 py-2 rounded-none text-[9px] font-sans font-bold transition-all ${
                          protocoloActivo === m 
                            ? 'bg-[#2B579A] text-white' 
                            : 'text-[#999] hover:text-[#2B579A]'
                        } ${!esKardexVacio ? 'cursor-not-allowed' : ''}`}
                      >
                        {m}
                      </button>
                    ))}
                  ...
                  {!esKardexVacio && (
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <p className="text-[8px] font-sans font-bold text-[#F97316] uppercase tracking-tighter">
                        El método de valuación no puede cambiarse una vez iniciadas las operaciones
                      </p>
                    </div>
                  )}
                </div>
              ...
              {activoSeleccionadoId && (
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 bg-[#F8F9FA] border border-[#DEE2E6] p-6">
                  <div>
                    <p className="text-[8px] font-bold text-[#999] uppercase tracking-widest mb-1">Código_Articulo</p>
                    <p className="text-sm font-sans font-bold text-[#2B579A]">{maestroActivos.find(p => p.id === activoSeleccionadoId)?.codigo}</p>
                  </div>
                  <div>
                    <p className="text-[8px] font-bold text-[#999] uppercase tracking-widest mb-1">Descripción_Detallada</p>
                    <p className="text-sm font-sans font-bold text-[#333]">{maestroActivos.find(p => p.id === activoSeleccionadoId)?.nombre}</p>
                  </div>
                  <div>
                    <p className="text-[8px] font-bold text-[#999] uppercase tracking-widest mb-1">Unidad_Medida</p>
                    <p className="text-sm font-sans font-bold text-[#333]">{maestroActivos.find(p => p.id === activoSeleccionadoId)?.unidadMedida}</p>
                  </div>
                  <div>
                    <p className="text-[8px] font-bold text-[#999] uppercase tracking-widest mb-1">Método_Valoración</p>
                    <p className="text-sm font-sans font-bold text-[#F97316]">{protocoloActivo}</p>
                  </div>
                </div>
              )}
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-[#F8F9FA] text-[8px] uppercase tracking-[0.2em] text-[#999] font-sans font-bold border-b border-[#DEE2E6]">
                      ...
                      <tr className="border-t border-[#DEE2E6] bg-gray-50">
                        <th className="px-4 py-3 border-r border-[#DEE2E6] text-[7px]">Tipo</th>
                        <th className="px-4 py-3 border-r border-[#DEE2E6] text-[7px]">Serie</th>
                        <th className="px-4 py-3 border-r border-[#DEE2E6] text-[7px]">Número</th>
                        <th className="px-4 py-3 border-r border-[#DEE2E6] text-[7px] text-center">CANT.</th>
                        <th className="px-4 py-3 border-r border-[#DEE2E6] text-[7px] text-center">UNIT.</th>
                        <th className="px-4 py-3 border-r border-[#DEE2E6] text-[7px] text-center">TOTAL.</th>
                        <th className="px-4 py-3 border-r border-[#DEE2E6] text-[7px] text-center">CANT.</th>
                        <th className="px-4 py-3 border-r border-[#DEE2E6] text-[7px] text-center">UNIT.</th>
                        <th className="px-4 py-3 border-r border-[#DEE2E6] text-[7px] text-center">TOTAL.</th>
                        <th className="px-4 py-3 border-r border-[#DEE2E6] text-[7px] text-center">CANT.</th>
                        <th className="px-4 py-3 border-r border-[#DEE2E6] text-[7px] text-center">UNIT.</th>
                        <th className="px-4 py-3 text-[7px] text-center">TOTAL.</th>
                      </tr>
                    </thead>
                    <tbody className="text-[10px] font-sans divide-y divide-[#DEE2E6]">
                      {activoSeleccionadoId ? (
                          ...
                          <tr key={idx} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-4 py-4 border-r border-[#DEE2E6]">
                              <div className={`text-[8px] font-bold uppercase tracking-wider ${
                                line.tipo === 'ENTRADA' ? 'text-[#2B579A]' : 'text-[#F97316]'
                              }`}>
                                {line.tipoOperacion || (line.tipo === 'ENTRADA' ? '02' : '01')}
                              </div>
                              <div className="text-[7px] text-[#999] uppercase truncate max-w-[80px]">
                                {getTabla12(line.tipoOperacion || (line.tipo === 'ENTRADA' ? '02' : '01'))}
                              </div>
                            </td>
                            {/* Entradas */}
                        ...
                        <tr>
                          <td colSpan={14} className="px-4 py-40 text-center text-[#999]">
                            <div className="max-w-xs mx-auto">
                              <div className="w-16 h-16 border border-[#DEE2E6] bg-[#F8F9FA] flex items-center justify-center mx-auto mb-6">
                                <Search size={32} className="text-[#DEE2E6]" />
                              </div>
                              <h5 className="font-sans font-bold text-xl text-[#2B579A] mb-2 uppercase tracking-tighter">Sin_Activo_Seleccionado</h5>
                              <p className="text-[10px] leading-relaxed uppercase tracking-tight">Seleccione un activo de la base de datos para inicializar la visualización del kardex.</p>
                            </div>
                          </td>
                        </tr>
                      )}
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead className="bg-white text-[8px] uppercase tracking-[0.2em] text-[#999] font-sans font-bold border-b border-[#DEE2E6]">
                        <tr>
                          <th className="px-8 py-4">Periodo_Mensual</th>
                          <th className="px-8 py-4">Saldo_Final_Uni</th>
                          <th className="px-8 py-4">Costo_Unitario_S/</th>
                          <th className="px-8 py-4">Valorización_Total_S/</th>
                          <th className="px-8 py-4 text-right">Estado_Cierre</th>
                        </tr>
                      </thead>
                      <tbody className="text-[10px] font-sans divide-y divide-[#DEE2E6]">
                        {resumenMensual.map((item, idx) => (
                          <tr key={idx} className="hover:bg-gray-50 transition-colors">
                            <td className="px-8 py-4 font-bold text-[#333] uppercase tracking-widest">
                              {format(new Date(item.periodo + '-02'), 'MMMM yyyy', { locale: es })}
                            </td>
                            ...
                            <td className="px-8 py-4 text-right">
                              <span className="bg-green-50 text-green-600 px-3 py-1 rounded-none text-[8px] font-black uppercase tracking-widest border border-green-100">
                                Transferido_OK
                              </span>
                            </td>
                          </tr>
          ...
          {activeTab === 'reports' && (
            ...
            >
              <Card className="p-10 group cursor-pointer border-[#DEE2E6] hover:border-[#2B579A]/50 transition-all">
                <div className="w-14 h-14 border border-[#DEE2E6] bg-[#F8F9FA] text-[#2B579A] flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-300">
                  <FileText size={28} />
                </div>
                ...
                <div className="space-y-4">
                  <select 
                    value={activoSeleccionadoId}
                    onChange={(e) => setActivoSeleccionadoId(e.target.value)}
                    className="w-full bg-white border border-[#DEE2E6] rounded-none px-4 py-3 text-[11px] font-sans text-[#333] focus:outline-none focus:border-[#2B579A]"
                  >
                    <option value="">-- SELECCIONAR ACTIVO --</option>
                    {maestroActivos.map(p => (
                      <option key={p.id} value={p.id}>{p.codigo} - {p.nombre}</option>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button 
                      ...
                      className="py-4 bg-[#2B579A] text-white rounded-none font-sans font-bold text-[10px] uppercase tracking-widest hover:bg-[#1A3A6A] disabled:opacity-20 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
                    >
                      PDF 12.1
                      <Download size={14} />
                    </button>
                    <button 
                      onClick={exportToExcelSUNAT121}
                      disabled={!activoSeleccionadoId}
                      className="py-4 border border-[#DEE2E6] bg-white text-[#333] rounded-none font-sans font-bold text-[10px] uppercase tracking-widest hover:bg-[#2B579A] hover:text-white disabled:opacity-20 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
                    >
                      XLS 12.1
                      <Download size={14} />
                    </button>
                    <button 
                      onClick={exportToPDFValorizado}
                      disabled={!activoSeleccionadoId}
                      className="py-4 bg-[#2B579A] text-white rounded-none font-sans font-bold text-[10px] uppercase tracking-widest hover:bg-[#1A3A6A] disabled:opacity-20 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
                    >
                      PDF 13.1
              ...
              <Card className="p-10 group cursor-pointer border-[#DEE2E6] hover:border-[#2B579A]/50 transition-all">
                ...
                <div className="grid grid-cols-1 gap-3">
                  <button 
                    onClick={exportMaestroArticulosPDF}
                    className="py-4 border border-[#DEE2E6] bg-white text-[#333] rounded-none font-sans font-bold text-[10px] uppercase tracking-widest hover:bg-[#2B579A] hover:text-white transition-all flex items-center justify-center gap-2"
                  >
                    Registro_Maestro_PDF
                    <FileText size={14} />
                  </button>
                  <button 
                    onClick={exportMaestroArticulosExcel}
                  ...
                  <button 
                    ...
                    className="py-4 border border-[#DEE2E6] bg-white text-[#333] rounded-none font-sans font-bold text-[10px] uppercase tracking-widest hover:bg-[#2B579A] hover:text-white transition-all flex items-center justify-center gap-2"
                  >
                    Consola_de_Alertas_Stock
                    <AlertTriangle size={14} />
                  </button>
                  <button 
                    onClick={() => setShowMovementReportModal(true)}
                    className="py-4 border border-[#DEE2E6] bg-white text-[#333] rounded-none font-sans font-bold text-[10px] uppercase tracking-widest hover:bg-[#2B579A] hover:text-white transition-all flex items-center justify-center gap-2"
                  >
                    Consola_de_Logs_Movimientos
                    <History size={14} />
                  </button>
                  <button 
              ...
              <Card className="p-10 group cursor-pointer border-[#DEE2E6] hover:border-[#2B579A]/50 transition-all">
                <div className="w-14 h-14 border border-[#DEE2E6] bg-[#F8F9FA] text-[#F97316] flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-300">
                ...
                <button 
                  onClick={exportStockAlertsReport}
                  className="w-full py-4 bg-orange-50 border border-orange-200 text-[#F97316] rounded-none font-sans font-bold text-[10px] uppercase tracking-widest hover:bg-[#F97316] hover:text-white transition-all flex items-center justify-center gap-3"
                >
                  Exportar_Log_Alertas
                  <Download size={16} />
                </button>
              </Card>
              ...
              <Card className="p-10 group cursor-pointer border-[#DEE2E6] hover:border-[#2B579A]/50 transition-all">
                ...
                <button 
                  onClick={exportToExcel}
                  className="w-full py-4 border border-[#DEE2E6] bg-white text-[#333] rounded-none font-sans font-bold text-[10px] uppercase tracking-widest hover:bg-[#2B579A] hover:text-white transition-all flex items-center justify-center gap-3"
                >
              ...
              <Card className="p-10 group cursor-pointer border-[#DEE2E6] hover:border-[#2B579A]/50 transition-all">
                ...
                <button 
                  onClick={exportLowMovementReport}
                  className="w-full py-4 border border-[#DEE2E6] bg-white text-[#333] rounded-none font-sans font-bold text-[10px] uppercase tracking-widest hover:bg-[#2B579A] hover:text-white transition-all flex items-center justify-center gap-3"
                >
                  Exportar_Reporte_Rotación
                  <Download size={16} />
                </button>
              </Card>
            </motion.div>
              ...
              <Card className="p-10">
                ...
                <div className="space-y-8">
                  <div>
                    ...
                    <div className="space-y-4 bg-[#F8F9FA] p-6 border border-[#DEE2E6]">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[9px] font-bold text-[#999] uppercase mb-1">Nombre</label>
                          <input 
                            type="text"
                            value={empresaConfig.nombre}
                            onChange={(e) => setEmpresaConfig({...empresaConfig, nombre: e.target.value})}
                            className="w-full bg-white border border-[#DEE2E6] px-3 py-2 text-xs focus:outline-none focus:border-[#2B579A]"
                          />
                        </div>
                        <div>
                          <label className="block text-[9px] font-bold text-[#999] uppercase mb-1">RUC</label>
                          <input 
                            type="text"
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[9px] font-bold text-[#999] uppercase mb-1">Teléfono</label>
                          <input 
                            type="text"
                        ...
                        <div>
                          <label className="block text-[9px] font-bold text-[#999] uppercase mb-1">Dirección</label>
                          <input 
                            type="text"
                      ...
                      <button 
                        onClick={() => guardarEmpresa(empresaConfig)}
                        className="bg-[#2B579A] text-white px-4 py-2 text-[10px] font-bold uppercase tracking-widest hover:bg-[#1A3A6A] transition-all"
                      >
                        Actualizar Datos
                      </button>
                    </div>
                  ...
                  <div>
                    ...
                    <div className={`flex bg-[#F8F9FA] p-1 rounded-none border border-[#DEE2E6] w-fit ${!esKardexVacio ? 'opacity-70 grayscale-[0.5]' : ''}`}>
                      {(['PROMEDIO', 'PEPS'] as ProtocoloValuacion[]).map((m) => (
                        <button
                          ...
                          disabled={!esKardexVacio}
                          onClick={() => setProtocoloActivo(m)}
                          className={`px-6 py-2.5 rounded-none text-[10px] font-bold transition-all ${
                            protocoloActivo === m 
                    ...
                    {!esKardexVacio ? (
                      <div className="flex items-center gap-2 mt-3 p-3 bg-orange-50 border border-orange-100">
                        <AlertTriangle size={14} className="text-[#F97316]" />
                        <p className="text-[10px] font-bold text-[#F97316] uppercase tracking-tight">
                          El método de valuación no puede cambiarse una vez iniciadas las operaciones
                        </p>
                      </div>
                    ) : (
                      <p className="mt-3 text-[10px] text-[#999] leading-relaxed">
                        Este método se aplicará globalmente a todos los cálculos de Kardex y Dashboard.
                      </p>
                    )}
                  </div>
                  ...
                  <div className="pt-8 border-t border-[#DEE2E6]">
                    ...
                    <p className="text-xs text-[#666] leading-relaxed mb-4">
                      Control de inventarios bajo normativa SUNAT (Perú).
                    </p>
                    <div className="flex gap-4 text-[10px] font-bold text-[#999]">
                      <span>Versión 1.2.0</span>
                      <span>•</span>
                      <span className="text-[#F97316]">Gestión de Almacén</span>
                    </div>
                  </div>

                  <div className="pt-8 border-t border-[#DEE2E6]">
                    <button 
                      onClick={purgarBaseDatos}
                      className="px-6 py-3 bg-orange-50 text-[#F97316] rounded-none border border-orange-100 text-[10px] font-bold uppercase tracking-widest hover:bg-orange-100 transition-all"
                    >
      ...
      <AnimatePresence>
        ...
        {showStockAlertModal && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[60] p-6"
          >
            <motion.div 
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white border border-[#DEE2E6] p-10 max-w-4xl w-full shadow-2xl max-h-[80vh] overflow-y-auto rounded-none relative"
            >
              <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-[#F97316]"></div>
              ...
              <div className="flex justify-between items-center mb-10">
                <div className="flex items-center gap-5">
                  <div className="w-14 h-14 border border-orange-100 bg-orange-50 text-[#F97316] flex items-center justify-center">
                    <AlertTriangle size={28} />
                  </div>
                  <div>
                    <h3 className="text-2xl font-sans font-bold tracking-tighter text-[#2B579A] uppercase">Alertas_Críticas_de_Stock</h3>
                    <p className="text-[#999] text-[10px] font-sans uppercase tracking-widest font-bold">Activos por debajo del umbral de seguridad operacional</p>
                  </div>
                </div>
                <button onClick={() => setShowStockAlertModal(false)} className="p-3 border border-[#DEE2E6] text-[#999] hover:text-[#2B579A] hover:border-[#2B579A]/50 transition-all">
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="border border-[#DEE2E6] overflow-hidden">
                <table className="w-full text-left border-collapse">
                  <thead className="bg-[#F8F9FA] text-[10px] uppercase tracking-widest text-[#999] font-bold border-b border-[#DEE2E6]">
                    <tr>
                      <th className="px-6 py-5">ID_Código</th>
                      <th className="px-6 py-5">Nombre_Activo</th>
                      <th className="px-6 py-5 text-center">Mín_Seguridad</th>
                      <th className="px-6 py-5 text-center">Nivel_Actual</th>
                      <th className="px-6 py-5 text-center">Flag_Estado</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#DEE2E6] text-[12px] font-sans">
                    {maestroActivos.filter(p => obtenerSaldoFisico(p.id) <= p.stockMinimo).map(p => {
                      const stock = obtenerSaldoFisico(p.id);
                      return (
                        <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-5 text-[#999]">{p.codigo}</td>
                          <td className="px-6 py-5 text-[#333] font-bold uppercase">{p.nombre}</td>
                          <td className="px-6 py-5 text-center text-[#999]">{p.stockMinimo}</td>
                          <td className="px-6 py-5 text-center font-bold text-[#F97316] text-lg">{stock}</td>
                          <td className="px-6 py-5 text-center">
                            <span className="px-3 py-1 border border-orange-200 bg-orange-50 text-[#F97316] text-[9px] font-bold uppercase tracking-widest">
                    ...
                    {maestroActivos.filter(p => obtenerSaldoFisico(p.id) <= p.stockMinimo).length === 0 && (
                      <tr>
                        <td colSpan={5} className="px-6 py-24 text-center text-[#999] uppercase text-[10px] tracking-[0.2em] font-sans">
        ...
        {showMovementReportModal && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white border border-[#DEE2E6] p-10 max-w-5xl w-full shadow-2xl max-h-[80vh] overflow-y-auto rounded-none relative"
            >
              <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-[#F97316]"></div>
              <div className="absolute top-0 right-0 w-2 h-2 border-t border-r border-[#F97316]"></div>
              <div className="absolute bottom-0 left-0 w-2 h-2 border-b border-l border-[#F97316]"></div>
              ...
              <div className="flex justify-between items-center mb-10">
                <div className="flex items-center gap-5">
                  <div className="w-14 h-14 border border-[#2B579A]/10 bg-[#E7EEF8] text-[#2B579A] flex items-center justify-center">
                    <History size={28} />
                  </div>
                  <div>
                    <h3 className="text-2xl font-sans font-bold tracking-tighter text-[#2B579A] uppercase">Log_de_Historial_Operativo</h3>
                    <p className="text-[#999] text-[10px] font-sans uppercase tracking-widest font-bold">Registro cronológico completo del flujo de activos</p>
                  </div>
                </div>
                <button onClick={() => setShowMovementReportModal(false)} className="p-3 border border-[#DEE2E6] text-[#999] hover:text-[#2B579A] hover:border-[#2B579A]/50 transition-all">
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="border border-[#DEE2E6] overflow-hidden">
                <table className="w-full text-left border-collapse">
                  <thead className="bg-[#F8F9FA] text-[10px] uppercase tracking-widest text-[#999] font-bold border-b border-[#DEE2E6]">
                    <tr>
                      <th className="px-6 py-5">Marca_de_Tiempo</th>
                      <th className="px-6 py-5">Descriptor_de_Activo</th>
                      <th className="px-6 py-5">Tipo_Op</th>
                      <th className="px-6 py-5">Referencia_Doc</th>
                      <th className="px-6 py-5 text-right">Cant</th>
                      <th className="px-6 py-5 text-right">Val_Unit</th>
                      <th className="px-6 py-5 text-right">Val_Total</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#DEE2E6] text-[12px] font-sans">
                    {[...bitacoraOperaciones].reverse().map(m => {
                      const product = maestroActivos.find(p => p.id === m.activoId);
                      return (
                        <tr key={m.id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-5 text-[#999]">{m.fecha}</td>
                          <td className="px-6 py-5 text-[#333] font-bold uppercase">{product?.nombre || 'REF_NULA'}</td>
                          <td className="px-6 py-5">
                            <span className={`px-2 py-1 border text-[9px] font-bold uppercase tracking-widest ${
                              m.tipo === 'ENTRADA' ? 'border-[#2B579A]/20 bg-[#E7EEF8] text-[#2B579A]' : 'border-orange-200 bg-orange-50 text-[#F97316]'
                            }`}>
                              {m.tipo}
                            </span>
                          </td>
                          <td className="px-6 py-5 text-[#999]">{m.tipoDocumento} {m.serie}-{m.numero}</td>
                          <td className={`px-6 py-5 text-right font-bold ${m.tipo === 'ENTRADA' ? 'text-[#2B579A]' : 'text-[#F97316]'}`}>
                            {m.tipo === 'ENTRADA' ? '+' : '-'}{m.cantidad}
                          </td>
                          <td className="px-6 py-5 text-right text-[#999]">S/ {m.costoUnitario.toFixed(4)}</td>
                          <td className="px-6 py-5 text-right font-bold text-[#2B579A]">S/ {m.costoTotal.toLocaleString()}</td>
                        </tr>
                      );
                    })}
                    {bitacoraOperaciones.length === 0 && (
                      <tr>
                        <td colSpan={7} className="px-6 py-24 text-center text-[#999] uppercase text-[10px] tracking-[0.2em] font-sans">
                          [ NO SE ENCONTRARON DATOS OPERATIVOS ]
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            >
              ...
              <div className="flex justify-between items-center mb-10">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h3 className="text-2xl font-sans font-bold tracking-tighter text-[#2B579A] uppercase">Análisis_de_Baja_Rotación</h3>
                    <p className="text-[#999] text-[10px] font-sans uppercase tracking-widest font-bold">Activos estancados para el periodo: {format(new Date(), 'MMMM_yyyy', { locale: es }).toUpperCase()}</p>
                  </div>
                </div>
                <button onClick={() => setShowLowMovementModal(false)} className="p-3 border border-[#DEE2E6] text-[#999] hover:text-[#2B579A] hover:border-[#2B579A]/50 transition-all">
                  <X size={24} />
                </button>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="bg-[#F8F9FA] text-[10px] uppercase tracking-widest text-[#999] font-bold border-b border-[#DEE2E6]">
                    <tr>
                      <th className="px-6 py-5">ID_Código</th>
                      <th className="px-6 py-5">Descriptor_de_Activo</th>
                      <th className="px-6 py-5 text-center">Índice_de_Rotación</th>
                      <th className="px-6 py-5 text-center">Stock_Actual</th>
                      <th className="px-6 py-5 text-center">Aviso_del_Sistema</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#DEE2E6] text-[12px] font-sans">
                    {maestroActivos.map(p => {
                      const currentMonth = format(new Date(), 'yyyy-MM');
                      const monthMovements = bitacoraOperaciones.filter(m => 
                        m.activoId === p.id && m.fecha.startsWith(currentMonth)
                      ).length;
                      return { ...p, monthMovements };
                    })
                    .filter(p => p.monthMovements < 2)
                    .map(p => (
                      <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-5 text-[#999]">{p.codigo}</td>
                        <td className="px-6 py-5 text-[#333] font-bold uppercase">{p.nombre}</td>
                        <td className="px-6 py-5 text-center">
                          <span className={`font-bold ${p.monthMovements === 0 ? 'text-[#F97316]' : 'text-[#F97316]'}`}>
                            {p.monthMovements} OPS
                          </span>
                        </td>
                        <td className="px-6 py-5 text-center font-bold text-[#333]">{obtenerSaldoFisico(p.id)}</td>
                        <td className="px-6 py-5 text-center">
                          <span className="text-[9px] font-bold text-[#999] uppercase tracking-widest border border-[#DEE2E6] px-2 py-1">
                            {p.monthMovements === 0 ? 'LIQUIDAR_ACTIVO' : 'OPTIMIZAR_STOCK'}
                          </span>
                        </td>
                      </tr>
                    ))}
                    {maestroActivos.length > 0 && maestroActivos.filter(p => {
                      const currentMonth = format(new Date(), 'yyyy-MM');
                      const monthMovements = bitacoraOperaciones.filter(m => 
                        m.activoId === p.id && m.fecha.startsWith(currentMonth)
                      ).length;
                      return monthMovements < 2;
                    }).length === 0 && (
                      <tr>
                        <td colSpan={5} className="px-6 py-24 text-center text-[#999] uppercase text-[10px] tracking-[0.2em] font-sans">
                          [ TODOS LOS ACTIVOS CON ROTACIÓN ÓPTIMA ]
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        {deleteConfirm && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[60] p-6"
          >
            <motion.div 
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white border border-[#DEE2E6] p-10 max-w-md w-full shadow-2xl rounded-none relative"
            >
              <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-[#F97316]"></div>
              <div className="absolute top-0 right-0 w-2 h-2 border-t border-r border-[#F97316]"></div>
              <div className="absolute bottom-0 left-0 w-2 h-2 border-b border-l border-[#F97316]"></div>
              ...
              <div className="w-16 h-16 border border-orange-100 bg-orange-50 text-[#F97316] flex items-center justify-center mb-8">
              ...
              <div className="flex gap-4">
                <button 
                  ...
                  className="flex-1 py-4 border border-[#DEE2E6] text-[#666] font-bold text-[10px] uppercase tracking-widest hover:bg-[#F8F9FA] transition-all"
                >
                  ABORTAR_OP
                </button>
                <button 
                  onClick={confirmarEliminacionKhipu}
                  className="flex-1 py-4 bg-[#F97316] text-white font-bold text-[10px] uppercase tracking-widest hover:bg-[#EA580C] transition-all"
                >
        ...
        {(showProductModal || showMovementModal) && (
          ...
          >
            <motion.div 
              ...
              className={`bg-white border border-[#DEE2E6] w-full ${showMovementModal ? 'max-w-4xl' : 'max-w-lg'} rounded-none shadow-2xl overflow-hidden relative transition-all duration-500`}
            >
              ...
              <div className="p-8 border-b border-[#DEE2E6] flex justify-between items-center bg-[#F8F9FA]">
                <div>
                  <h3 className="text-2xl font-display font-bold tracking-tighter text-[#1A1A1A] uppercase">
                    {showProductModal ? 'Registro de Activos' : 'Entrada de Operación Multi-Item'}
                  </h3>
                  <p className="text-[10px] font-bold text-[#999] uppercase tracking-[0.2em] mt-1">
                    {showProductModal ? 'Sistema de Catálogo v1.2' : 'Interfaz de Kardex v1.5 - Multi-Item'}
                  </p>
                </div>
                <button 
                  onClick={() => { setShowProductModal(false); setShowMovementModal(false); setStagedItems([]); }}
                  className="p-3 border border-[#DEE2E6] text-[#999] hover:text-[#2B579A] hover:border-[#2B579A]/50 transition-all"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-8">
                {showProductModal ? (
                  <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          <input 
                            ...
                            placeholder="P-000"
                            className="flex-1 bg-[#F8F9FA] border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none px-4 py-3 text-sm focus:outline-none focus:border-[#2B579A] transition-all"
                          />
                          <button 
                            onClick={() => setNuevoActivo({ ...nuevoActivo, codigo: recomendarCodigo() })}
                            className="px-3 bg-white border border-[#DEE2E6] text-[#2B579A] hover:bg-[#E7EEF8] transition-all text-[9px] font-bold uppercase tracking-tighter"
                            title="Recomendar código"
                          >
                            Auto
                          </button>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        ...
                        <select 
                          value={nuevoActivo.tipoExistencia}
                          onChange={(e) => setNuevoActivo({...nuevoActivo, tipoExistencia: e.target.value})}
                          className="w-full bg-[#F8F9FA] border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none px-4 py-3 text-sm focus:outline-none focus:border-[#2B579A] transition-all"
                        >
                          <option value="01">01 - MERCADERÍA</option>
                          <option value="02">02 - PRODUCTO TERMINADO</option>
                          <option value="03">03 - MATERIAS PRIMAS Y AUXILIARES - MATERIALES</option>
                          <option value="04">04 - ENVASES Y EMBALAJES</option>
                          <option value="05">05 - SUMINISTROS DIVERSOS</option>
                          <option value="99">99 - OTROS (ESPECIFICAR)</option>
                        </select>
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-[#999] uppercase tracking-widest mb-2">Unidad Medida (Tabla 6)</label>
                        <select 
                          value={nuevoActivo.unidadMedidaCodigo}
                          onChange={(e) => {
                            const label = e.target.options[e.target.selectedIndex].text.split(' - ')[1];
                            setNuevoActivo({...nuevoActivo, unidadMedidaCodigo: e.target.value, unidadMedida: label});
                          }}
                          className="w-full bg-[#F8F9FA] border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none px-4 py-3 text-sm focus:outline-none focus:border-[#2B579A] transition-all"
                        >
                          <option value="01">01 - KILOGRAMOS</option>
                          <option value="02">02 - LIBRAS</option>
                          <option value="03">03 - TONELADAS LARGAS</option>
                          <option value="04">04 - TONELADAS MÉTRICAS</option>
                          <option value="05">05 - TONELADAS CORTAS</option>
                          <option value="06">06 - GRAMOS</option>
                          <option value="07">07 - UNIDADES</option>
                          <option value="08">08 - LITROS</option>
                          <option value="09">09 - GALONES</option>
                          <option value="10">10 - BARRILES</option>
                          <option value="11">11 - LATAS</option>
                          <option value="12">12 - CAJAS</option>
                          <option value="13">13 - MILLARES</option>
                          <option value="14">14 - METROS CÚBICOS</option>
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        placeholder="Nombre_Activo_Sistema"
                        className="w-full bg-[#F8F9FA] border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none px-4 py-3 text-sm focus:outline-none focus:border-[#2B579A] transition-all"
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        <label className="block text-[10px] font-bold text-[#999] uppercase tracking-widest mb-2">Mín_Seguridad</label>
                        <input 
                          type="number" 
                          value={isNaN(nuevoActivo.stockMinimo as number) ? '' : nuevoActivo.stockMinimo}
                          onChange={(e) => setNuevoActivo({...nuevoActivo, stockMinimo: e.target.value === '' ? 0 : parseInt(e.target.value)})}
                          className="w-full bg-[#F8F9FA] border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none px-4 py-3 text-sm focus:outline-none focus:border-[#2B579A] transition-all"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-[#999] uppercase tracking-widest mb-2">Máx_Seguridad</label>
                        <input 
                          type="number" 
                          value={isNaN(nuevoActivo.stockMaximo as number) ? '' : nuevoActivo.stockMaximo}
                          onChange={(e) => setNuevoActivo({...nuevoActivo, stockMaximo: e.target.value === '' ? 0 : parseInt(e.target.value)})}
                          className="w-full bg-[#F8F9FA] border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none px-4 py-3 text-sm focus:outline-none focus:border-[#2B579A] transition-all"
                        />
                      </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-[#DEE2E6]">
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        <div>
                          <label className="block text-[10px] font-bold text-[#999] uppercase tracking-widest mb-2">Stock_Inicial</label>
                          <input 
                            type="number" 
                            value={nuevoActivo.stockInicial}
                            onChange={(e) => setNuevoActivo({...nuevoActivo, stockInicial: e.target.value})}
                            className="w-full bg-[#F8F9FA] border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none px-4 py-3 text-sm focus:outline-none focus:border-[#2B579A] transition-all"
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-bold text-[#999] uppercase tracking-widest mb-2">Costo_Unit_Inicial</label>
                          <input 
                            type="number" 
                            value={nuevoActivo.costoInicial}
                            onChange={(e) => setNuevoActivo({...nuevoActivo, costoInicial: e.target.value})}
                            className="w-full bg-[#F8F9FA] border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none px-4 py-3 text-sm focus:outline-none focus:border-[#2B579A] transition-all"
                          />
                        </div>
                      </div>
                    ...
                    <div className="flex gap-4 pt-6 border-t border-[#DEE2E6]">
                      <button 
                        ...
                        className="flex-1 py-4 border border-[#DEE2E6] text-[#999] font-bold text-[10px] uppercase tracking-widest hover:bg-white transition-all shadow-sm"
                      >
                        CANCELAR
                      </button>
                      <button 
                        onClick={registrarNuevoActivo}
                        disabled={!nuevoActivo.codigo || !nuevoActivo.nombre}
                        className={`flex-1 py-4 text-white font-bold text-[10px] uppercase tracking-widest transition-all shadow-lg ${
                          !nuevoActivo.codigo || !nuevoActivo.nombre
                            ? 'bg-gray-400 cursor-not-allowed'
                            : 'bg-[#2B579A] hover:bg-[#1E3E6D] hover:scale-[1.02] active:scale-[0.98]'
                        }`}
                      >
                        REGISTRAR_ACTIVO
                      </button>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    {/* COLUMNA IZQUIERDA: CABECERA Y SELECCIÓN */}
                    <div className="lg:col-span-5 space-y-6">
                      <div className="space-y-4 bg-[#F8F9FA] p-5 border border-[#DEE2E6]">
                        <h4 className="text-[10px] font-bold text-[#2B579A] uppercase tracking-widest flex items-center gap-2">
                          <FileText size={14} /> Información del Comprobante
                        </h4>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="block text-[9px] font-bold text-[#999] uppercase mb-1">Documento</label>
                            <select 
                              value={nuevaOperacion.tipoDocumento}
                              onChange={(e) => setNuevaOperacion({...nuevaOperacion, tipoDocumento: e.target.value})}
                              className="w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none px-2 py-2 text-[11px] focus:outline-none focus:border-[#2B579A]"
                            >
                              <option value="01">01 - FACTURA</option>
                              <option value="03">03 - BOLETA</option>
                              <option value="09">09 - GUÍA REM.</option>
                              {nuevaOperacion.tipo === 'SALIDA' && (
                                <option value="50">50 - DOCUMENTO INTERNO</option>
                              )}
                              <option value="00">00 - OTROS</option>
                            </select>
                          </div>
                          <div>
                            <label className="block text-[9px] font-bold text-[#999] uppercase mb-1">Fecha</label>
                            <input 
                              type="date" 
                              value={nuevaOperacion.fecha}
                              onChange={(e) => setNuevaOperacion({...nuevaOperacion, fecha: e.target.value})}
                              className="w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none px-2 py-2 text-[11px] focus:outline-none"
                            />
                          </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <input 
                            ...
                            value={nuevaOperacion.serie}
                            onChange={(e) => setNuevaOperacion({...nuevaOperacion, serie: e.target.value})}
                            className="w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans px-3 py-2 text-[11px] focus:outline-none"
                          />
                          <input 
                            type="text" 
                            placeholder="Número"
                            value={nuevaOperacion.numero}
                            onChange={(e) => setNuevaOperacion({...nuevaOperacion, numero: e.target.value})}
                            className="w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans px-3 py-2 text-[11px] focus:outline-none"
                          />
                        </div>
                        <div className="grid grid-cols-2 gap-3 pt-2">
                          <button 
                            onClick={() => setNuevaOperacion({...nuevaOperacion, tipo: 'ENTRADA'})}
                            disabled={stagedItems.length > 0}
                            className={`py-2 rounded-none text-[9px] font-bold tracking-widest border transition-all ${
                              nuevaOperacion.tipo === 'ENTRADA' 
                                ? 'bg-[#E7EEF8] border-[#2B579A] text-[#2B579A]' 
                                : 'bg-white border-[#DEE2E6] text-[#999]'
                            } ${stagedItems.length > 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
                          >
                            ENTRADA
                          </button>
                          <button 
                            onClick={() => setNuevaOperacion({...nuevaOperacion, tipo: 'SALIDA'})}
                            disabled={stagedItems.length > 0}
                            className={`py-2 rounded-none text-[9px] font-bold tracking-widest border transition-all ${
                              nuevaOperacion.tipo === 'SALIDA' 
                      ...
                      <div className="space-y-4 border border-[#DEE2E6] p-5 relative">
                        <h4 className="text-[10px] font-bold text-[#2B579A] uppercase tracking-widest flex items-center gap-2">
                          <Package size={14} /> Seleccionar Item
                        </h4>
                        <div className="relative">
                          ...
                          <input 
                            ...
                            value={busquedaActivoModal}
                            onChange={(e) => setBusquedaActivoModal(e.target.value)}
                            className="w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none pl-9 pr-10 py-2 text-[11px] focus:outline-none focus:border-[#2B579A]"
                          />
                        </div>
                        <select 
                          value={activoSeleccionadoId}
                          onChange={(e) => setActivoSeleccionadoId(e.target.value)}
                          className="w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none px-3 py-2 text-[11px] focus:outline-none"
                        >
                          <option value="">-- Buscar activo --</option>
                          {maestroActivos
                            .filter(p => 
                              p.id === activoSeleccionadoId ||
                              (p.nombre || '').toLowerCase().includes(busquedaActivoModal.toLowerCase()) ||
                              (p.codigo || '').toLowerCase().includes(busquedaActivoModal.toLowerCase())
                            )
                            .map(p => (
                              <option key={p.id} value={p.id}>{p.codigo} - {p.nombre}</option>
                            ))}
                        ...
                        <div className="border border-[#DEE2E6] bg-[#F8F9FA] p-4 space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <label className="block text-[9px] font-bold text-[#666] uppercase mb-1">Cantidad</label>
                              <input 
                                type="text"
                                value={localCantidad}
                                onChange={(e) => handleCantidadChange(e.target.value)}
                                onFocus={() => setFocusedField('cantidad')}
                                onBlur={() => formatOnBlur('cantidad')}
                                placeholder="0"
                                className="w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans font-mono px-3 py-2 text-[11px] focus:outline-none focus:border-[#2B579A]"
                              />
                            </div>
                            <div>
                              <label className="block text-[9px] font-bold text-[#666] uppercase mb-1">¿Afecto a I.G.V.?</label>
                              <select 
                                value={localGrabado ? "true" : "false"}
                                onChange={(e) => handleGrabadoToggle(e.target.value === 'true')}
                                disabled={nuevaOperacion.tipo === 'SALIDA' && nuevaOperacion.tipoDocumento !== '01' && nuevaOperacion.tipoDocumento !== '03'}
                                className={`w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans px-3 py-2 text-[11px] focus:outline-none focus:border-[#2B579A] ${(nuevaOperacion.tipo === 'SALIDA' && nuevaOperacion.tipoDocumento !== '01' && nuevaOperacion.tipoDocumento !== '03') ? 'opacity-40 cursor-not-allowed bg-gray-50' : ''}`}
                              >
                                <option value="true">Sí (Gravado 18%)</option>
                                <option value="false">No (Exonerado 0%)</option>
                              </select>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <label className="block text-[9px] font-bold text-[#666] uppercase mb-1">
                                Costo Unitario <span className="text-[8px] text-gray-400 font-normal">(Sin IGV)</span>
                              </label>
                              <input 
                                type="text"
                                value={localCostoSinIGV}
                                onChange={(e) => handleCostoSinIGVChange(e.target.value)}
                                onFocus={() => setFocusedField('costoSin')}
                                onBlur={() => formatOnBlur('costoSin')}
                                disabled={nuevaOperacion.tipo === 'SALIDA'}
                                placeholder="0.0000"
                                className={`w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans font-mono px-3 py-2 text-[11px] focus:outline-none focus:border-[#2B579A] ${nuevaOperacion.tipo === 'SALIDA' ? 'opacity-40 cursor-not-allowed bg-gray-50' : ''}`}
                              />
                            </div>
                            <div>
                              <label className="block text-[9px] font-bold text-[#F97316] uppercase mb-1">
                                Unitario <span className="text-[8px] font-semibold text-orange-500">{localGrabado ? 'Con IGV' : 'Sin IGV (Exo)'}</span>
                              </label>
                              <input 
                                type="text"
                                value={localCostoConIGV}
                                onChange={(e) => handleCostoConIGVChange(e.target.value)}
                                onFocus={() => setFocusedField('costoCon')}
                                onBlur={() => formatOnBlur('costoCon')}
                                disabled={nuevaOperacion.tipo === 'SALIDA'}
                                placeholder="0.0000"
                                className={`w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans font-mono px-3 py-2 text-[11px] focus:outline-none focus:border-orange-500 ${nuevaOperacion.tipo === 'SALIDA' ? 'opacity-40 cursor-not-allowed bg-gray-50' : ''}`}
                              />
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            <div>
                              <label className="block text-[9px] font-bold text-[#2B579A] uppercase mb-1">
                                Importe Total <span className="text-[8px] font-semibold text-blue-500">{localGrabado ? 'Con IGV' : 'Sin IGV (Exo)'}</span>
                              </label>
                              <input 
                                type="text"
                                value={localTotalConIGV}
                                onChange={(e) => handleTotalConIGVChange(e.target.value)}
                                onFocus={() => setFocusedField('totalCon')}
                                onBlur={() => formatOnBlur('totalCon')}
                                disabled={nuevaOperacion.tipo === 'SALIDA'}
                                placeholder="0.00"
                                className={`w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans font-mono px-3 py-2 text-[11px] focus:outline-none focus:border-blue-500 ${nuevaOperacion.tipo === 'SALIDA' ? 'opacity-40 cursor-not-allowed bg-gray-50' : ''}`}
                              />
                            </div>
                          </div>
                          ...
                          <div className="pt-2 border-t border-dashed border-[#DEE2E6] text-[10px] space-y-1 text-gray-500 font-sans">
                            <div className="flex justify-between">
                              ...
                              <span className="font-mono text-gray-700">S/ {((parseFloat(localCantidad) || 0) * (parseFloat(localCostoSinIGV) || 0)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Monto I.G.V. ({localGrabado ? '18%' : 'Exonerado 0%'}):</span>
                              <span className={`font-mono ${localGrabado ? 'text-[#F97316]' : 'text-gray-400'}`}>
                                S/ {localGrabado ? (((parseFloat(localCantidad) || 0) * (parseFloat(localCostoSinIGV) || 0) * 0.18).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })) : '0.00'}
                              </span>
                            </div>
                          </div>
                        </div>
                        <button 
                          onClick={agregarPartidaAStaging}
                          className="w-full py-3 bg-[#E7EEF8] border border-[#2B579A] text-[#2B579A] font-bold text-[10px] uppercase tracking-widest hover:bg-[#D1E1F5] transition-all flex items-center justify-center gap-2"
                        >
                          <Plus size={14} /> AGREGAR_AL_DOC
                        </button>
                      </div>
                    </div>

                    {/* COLUMNA DERECHA: LISTA DE ITEMS Y RESUMEN */}
                    <div className="lg:col-span-7 flex flex-col h-full border border-[#DEE2E6] overflow-hidden">
                      <div className="bg-[#1A1A1A] text-white px-4 py-3 flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-3">
                          {stagedItems.length > 0 && (
                            <button 
                              onClick={descargarPDFComprobante}
                              className="flex items-center gap-2 text-[9px] font-bold text-[#F97316] hover:text-white transition-colors uppercase tracking-widest px-2 py-1 border border-[#F97316]/50 hover:border-white"
                              title="Descargar PDF"
                            >
                      ...
                      <div className="flex-1 overflow-auto bg-gray-50 min-h-[300px]">
                        {stagedItems.length === 0 ? (
                          <div className="h-full flex flex-col items-center justify-center text-[#999] space-y-3 p-10 opacity-40">
                            <ClipboardList size={40} strokeWidth={1} />
                            <p className="text-[10px] uppercase font-bold tracking-widest text-center">No hay partidas agregadas al comprobante</p>
                          </div>
                        ) : (
                          <table className="w-full text-left border-collapse">
                            <thead className="bg-[#F8F9FA] border-b border-[#DEE2E6]">
                              <tr className="text-[8px] font-bold text-[#666] uppercase tracking-tighter">
                                <th className="px-2 py-2">Cód</th>
                                <th className="px-2 py-2">Descripción</th>
                                <th className="px-2 py-2 text-right">Cant</th>
                                <th className="px-2 py-2 text-right">Costo Unit.</th>
                                <th className="px-2 py-2 text-center">Afectación</th>
                                <th className="px-2 py-2 text-right text-orange-600">Unit c/IGV</th>
                                <th className="px-2 py-2 text-right">Subtotal</th>
                                <th className="px-2 py-2 text-right text-orange-600 font-semibold">IGV (18%)</th>
                            ...
                            <tbody className="bg-white divide-y divide-[#DEE2E6]">
                              {stagedItems.map((item, idx) => {
                                ...
                                return (
                                  <tr key={idx} className="text-[10px] font-sans hover:bg-blue-50/30 transition-colors group">
                                    <td className="px-2 py-2 font-bold text-[#2B579A]">{item.activoCodigo}</td>
                                    <td className="px-2 py-2 text-[#333] max-w-[100px] truncate" title={item.activoNombre}>{item.activoNombre}</td>
                                    <td className="px-2 py-2 text-right font-mono">{item.cantidad}</td>
                                    <td className="px-2 py-2 text-right font-mono">S/ {item.costoUnitario.toFixed(4)}</td>
                                    <td className="px-2 py-2 text-center text-[9px] font-semibold">
                                      {isGrabado ? <span className="text-orange-600">GRAV. 18%</span> : <span className="text-gray-400">EXO. 0%</span>}
                                    </td>
                                    <td className="px-2 py-2 text-right font-mono text-orange-600">S/ {unitWithIGV.toFixed(4)}</td>
                                    <td className="px-2 py-2 text-right font-mono">S/ {subtotalItem.toFixed(2)}</td>
                                    <td className="px-2 py-2 text-right font-mono text-orange-600">S/ {igvItem.toFixed(2)}</td>
                                    <td className="px-2 py-2 text-right font-mono font-bold text-[#2B579A]">S/ {totalItem.toFixed(2)}</td>
                                    <td className="px-2 py-2 text-right">
                                      <button 
                                        onClick={() => removerPartidaDeStaging(idx)}
                      ...
                      <div className="bg-[#F8F9FA] border-t border-[#DEE2E6] p-5 space-y-2 text-[11px]">
                        <div className="flex justify-between items-center text-gray-600">
                          <span className="font-bold uppercase tracking-wider text-[9px]">Op. Gravada:</span>
                          <span className="font-mono font-bold">
                            S/ {stagedItems.filter(i => i.igvGrabado !== false).reduce((sum, i) => sum + i.costoTotal, 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </span>
                        </div>
                        <div className="flex justify-between items-center text-gray-600">
                          <span className="font-bold uppercase tracking-wider text-[9px]">Op. No Gravada (Exo):</span>
                          <span className="font-mono font-bold text-gray-400">
                            S/ {stagedItems.filter(i => i.igvGrabado === false).reduce((sum, i) => sum + i.costoTotal, 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </span>
                        </div>
                        <div className="flex justify-between items-center text-gray-600">
                          <span className="font-bold uppercase tracking-wider text-[9px]">I.G.V. Total (18%):</span>
                          <span className="font-mono font-bold text-orange-600">
                            S/ {stagedItems.filter(i => i.igvGrabado !== false).reduce((sum, i) => sum + (Math.round((i.costoTotal * 0.18 + Number.EPSILON) * 100) / 100), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </span>
                        </div>
                        <div className="flex justify-between items-center pt-2 border-t border-dashed border-[#DEE2E6] mb-4">
                          ...
                          <span className="text-2xl font-display font-bold text-[#2B579A] tracking-tighter">
                            S/ {stagedItems.reduce((sum, i) => {
                              const isGrabado = i.igvGrabado !== false;
                              const sub = i.costoTotal;
                              const igvVal = isGrabado ? Math.round((sub * 0.18 + Number.EPSILON) * 100) / 100 : 0;
                              return sum + sub + igvVal;
                        ...
                        <div className="flex gap-4">
                          <button 
                            onClick={() => { setShowMovementModal(false); setStagedItems([]); }}
                            className="flex-1 py-4 border border-[#DEE2E6] text-[#999] font-bold text-[10px] uppercase tracking-widest hover:bg-white transition-all shadow-sm"
                          >
                            CANCELAR
                          </button>
                          <button 
                            onClick={asentarOperacionKardex}
                            disabled={stagedItems.length === 0 || !nuevaOperacion.numero}
                            className={`flex-1 py-4 text-white font-bold text-[10px] uppercase tracking-widest transition-all shadow-lg ${
                              stagedItems.length === 0 || !nuevaOperacion.numero
                                ? 'bg-gray-400 cursor-not-allowed'
                                : 'bg-[#2B579A] hover:bg-[#1E3E6D] hover:scale-[1.02] active:scale-[0.98]'
                            }`}
                          >
        ...
        {showAdjustmentModal && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.95, opacity: 0, y: 20 }}
              className="bg-white border border-[#DEE2E6] w-full max-w-5xl rounded-none shadow-2xl overflow-hidden relative flex flex-col max-h-[90vh]"
            >
              ...
              <div className="p-8 border-b border-[#DEE2E6] flex justify-between items-center bg-[#F8F9FA]">
                <div>
                  <h3 className="text-2xl font-display font-bold tracking-tighter text-[#1A1A1A] uppercase">
                    Toma Física & Ajuste de Inventario
                  </h3>
                  <p className="text-[10px] font-bold text-[#999] uppercase tracking-[0.2em] mt-1">
                    Auditoría de Sobrantes y Faltantes v1.1
                  </p>
                </div>
                <button 
                  onClick={() => setShowAdjustmentModal(false)}
                  className="p-3 border border-[#DEE2E6] text-[#999] hover:text-[#2B579A] hover:border-[#2B579A]/50 transition-all font-bold"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-8 overflow-y-auto flex-1 space-y-6">
                ...
                <div className="bg-orange-50/50 border border-orange-200/60 p-5 grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div>
                    <label className="block text-[9px] font-bold text-orange-700 uppercase mb-1">Serie del Ajuste</label>
                    <input 
                      type="text" 
                      value={adjustmentSerie} 
                      onChange={(e) => setAdjustmentSerie(e.target.value.toUpperCase())}
                      className="w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-mono px-3 py-2 text-xs focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[9px] font-bold text-orange-700 uppercase mb-1">Número de Comprobante</label>
                    <input 
                      type="text" 
                      value={adjustmentNumero} 
                      onChange={(e) => setAdjustmentNumero(e.target.value)}
                      placeholder="Autogenerado"
                      className="w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-mono px-3 py-2 text-xs focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[9px] font-bold text-orange-700 uppercase mb-1">Fecha de Operación</label>
                    <input 
                      type="date" 
                      value={adjustmentFecha} 
                      onChange={(e) => setAdjustmentFecha(e.target.value)}
                      className="w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] px-3 py-2 text-xs focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[9px] font-bold text-orange-700 uppercase mb-1">Observaciones</label>
                    <input 
                      type="text" 
                      value={adjustmentObs} 
                ...
                <div className="relative">
                  ...
                  <input 
                    type="text" 
                    placeholder="Filtrar por código o nombre para auditar..."
                    value={filterAdjustmentSearch}
                    onChange={(e) => setFilterAdjustmentSearch(e.target.value)}
                    className="w-full bg-white border border-[#DEE2E6] text-[#1A1A1A] font-sans rounded-none pl-9 pr-4 py-2 text-xs focus:outline-none focus:border-[#2B579A]"
                  />
                </div>
                ...
                <div className="border border-[#DEE2E6] overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-[#F8F9FA] border-b border-[#DEE2E6]">
                      <tr className="text-[9px] font-bold text-[#666] uppercase tracking-wider">
                        <th className="px-4 py-3">Código</th>
                        <th className="px-4 py-3">Descripción de Existencias</th>
                        <th className="px-4 py-3 text-center">Stock Sistema</th>
                        <th className="px-4 py-3 text-center w-40">Conteo Físico Real</th>
                        <th className="px-4 py-3 text-center">Diferencia</th>
                        <th className="px-4 py-3 text-center">Tipo de Ajuste</th>
                        <th className="px-4 py-3 text-right">Ajuste Manual</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-[#DEE2E6] text-[11px] font-sans">
                      {maestroActivos
                        .filter(p => !filterAdjustmentSearch || p.nombre.toLowerCase().includes(filterAdjustmentSearch.toLowerCase()) || p.codigo.toLowerCase().includes(filterAdjustmentSearch.toLowerCase()))
                        .map(p => {
                          const stockSistema = obtenerSaldoFisico(p.id);
                          const conteo = physicalCounts[p.id] !== undefined ? physicalCounts[p.id] : stockSistema;
                          const diferencia = conteo - stockSistema;
                            ...
                            <tr key={p.id} className="hover:bg-gray-50/50 transition-colors">
                              ...
                              <td className="px-4 py-3 text-center">
                                <div className="flex items-center justify-center gap-1">
                                  <button 
                                    onClick={() => setPhysicalCounts(prev => ({...prev, [p.id]: Math.max(0, (prev[p.id] || 0) - 1)}))}
                                    className="px-2 py-1 border border-[#DEE2E6] bg-gray-100 hover:bg-gray-200 text-xs font-bold font-mono"
                                  >
                                    -
                                  </button>
                                  <input 
                                    ...
                                    onChange={(e) => {
                                      ...
                                      setPhysicalCounts(prev => ({...prev, [p.id]: isNaN(val) ? 0 : val}));
                                    }}
                                    className="w-16 text-center border border-[#DEE2E6] py-1 font-mono font-bold focus:outline-none"
                                  />
                                  <button 
                                    onClick={() => setPhysicalCounts(prev => ({...prev, [p.id]: (prev[p.id] || 0) + 1}))}
                                    className="px-2 py-1 border border-[#DEE2E6] bg-gray-100 hover:bg-gray-200 text-xs font-bold font-mono"
                                  >
                                    +
                                  </button>
                              ...
                              <td className="px-4 py-3 text-center font-mono font-bold">
                                <span className={diferencia > 0 ? 'text-green-600' : diferencia < 0 ? 'text-orange-600' : 'text-gray-400'}>
                                  {diferencia > 0 ? `+${diferencia}` : diferencia}
                                </span>
                              </td>
                              <td className="px-4 py-3 text-center">
                                {diferencia > 0 ? (
                                  <span className="inline-block px-3 py-1 bg-green-50 text-green-700 border border-green-200 font-bold uppercase text-[9px] tracking-wide">
                                    SOBRANTE (Ingreso)
                                  </span>
                                ) : diferencia < 0 ? (
                                  <span className="inline-block px-3 py-1 bg-orange-50 text-orange-700 border border-orange-200 font-bold uppercase text-[9px] tracking-wide">
                                    FALTANTE (Salida)
                                  </span>
                                ) : (
                                  <span className="inline-block px-3 py-1 bg-gray-50 text-gray-400 border border-gray-100 uppercase text-[9px] tracking-wide">
                                    Ok / Conforme
                                  </span>
                                )}
                              </td>
                              <td className="px-4 py-3 text-right">
                                {diferencia !== 0 && (
                                  <button 
                                    onClick={() => agregarAjusteAManualStaging(p.id, diferencia)}
                                    className="text-[9px] font-bold text-[#2B579A] border border-[#2B579A]/30 px-3 py-1 hover:bg-[#2B579A] hover:text-white transition-all uppercase tracking-tight"
                                    title="Enviar al borrador de comprobante de ajuste para revisar o asentar manualmente"
                                  >
                                    Ajustar Manual
                                  </button>
                                )}
              ...
              <div className="p-8 border-t border-[#DEE2E6] bg-[#F8F9FA] flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex gap-4 text-xs">
                  <div className="bg-green-50 text-green-800 px-4 py-2 border border-green-200 font-bold">
                    SOBRANTES: {maestroActivos.filter(p => {
                      const stockSistema = obtenerSaldoFisico(p.id);
                      const conteo = physicalCounts[p.id] !== undefined ? physicalCounts[p.id] : stockSistema;
                      return conteo > stockSistema;
                    }).length}
                  </div>
                  <div className="bg-orange-50 text-orange-800 px-4 py-2 border border-orange-200 font-bold">
                    FALTANTES: {maestroActivos.filter(p => {
                      const stockSistema = obtenerSaldoFisico(p.id);
                      const conteo = physicalCounts[p.id] !== undefined ? physicalCounts[p.id] : stockSistema;
                      return conteo < stockSistema;
                    }).length}
                ...
                <div className="flex gap-4">
                  <button 
                    onClick={() => setShowAdjustmentModal(false)}
                    className="px-6 py-4 border border-[#DEE2E6] bg-white text-[#999] hover:bg-gray-50 text-[10px] uppercase font-bold tracking-widest transition-all"
                  >
                    CERRAR
                  </button>
                  <button 
                    onClick={ejecutarAjusteAutomatico}
                    className="px-8 py-4 bg-orange-600 hover:bg-orange-700 text-white text-[10px] uppercase font-bold tracking-widest transition-all shadow-md hover:scale-[1.02] active:scale-[0.98]"
                  >
                    PROCESAR AJUSTES AUTOMÁTICOS
                  </button>
                </div>
              </div>
            </motion.div>
